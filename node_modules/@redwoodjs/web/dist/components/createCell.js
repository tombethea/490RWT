...
function createCell(_ref) {
  ...
  function NamedCell(props) {
    ...
    if (global.__REDWOOD__PRERENDERING) {
      ...
      const queryInfo = queryCache[cacheKey]; // This is true when the graphql handler couldn't be loaded
      // So we fallback to the loading state

      if (queryInfo !== null && queryInfo !== void 0 && queryInfo.renderLoading) {
        loading = true;
      } else {
        if (queryInfo !== null && queryInfo !== void 0 && queryInfo.hasProcessed) {
          loading = false;
