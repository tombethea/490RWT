...
const interleaveStrategy = {
  ImportDeclaration(baseImport, extImport) {
    ...
    const importSpecifierEquality = (lhs, rhs) => {
      var _lhs$imported, _rhs$imported, _lhs$local, _rhs$local;

      return lhs.type === rhs.type && ((_lhs$imported = lhs.imported) === null || _lhs$imported === void 0 ? void 0 : _lhs$imported.name) === ((_rhs$imported = rhs.imported) === null || _rhs$imported === void 0 ? void 0 : _rhs$imported.name) && ((_lhs$local = lhs.local) === null || _lhs$local === void 0 ? void 0 : _lhs$local.name) == ((_rhs$local = rhs.local) === null || _rhs$local === void 0 ? void 0 : _rhs$local.name);
    };
    ...
    const defaultPosition = specs => (0, _some.default)(specs).call(specs, (0, _algorithms.nodeIs)('ImportDefaultSpecifier')) ? -1 : 0; // Rule 3: There can only be one wildcard import per statement, and wildcard imports cannot
...
const concatUniqueStrategy = {
  ArrayExpression(base, ext, eq) {
    eq || (eq = defaultEquality(base.elements, ext.elements));
    base.elements = _lodash.default.uniqWith([...base.elements, ...ext.elements], eq);
  },

  ObjectExpression(base, ext, eq) {
    eq || (eq = defaultEquality(base.properties, ext.properties));
...
function concatUnique(baseOrEq, ext) {
  ...
  if (arguments.length === 2) {
    ...
    requireStrategyExists(baseOrEq, ext, concatUniqueStrategy, 'concatUnique'); // The type-specific concatUnique implementations will provide an appropriate equality operator.
