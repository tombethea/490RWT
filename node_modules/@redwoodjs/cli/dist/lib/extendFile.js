...
function extendJSXFile(path, {
  ...
  if (moduleScopeLines !== null && moduleScopeLines !== void 0 && moduleScopeLines.length) {
    ...
    }) + 1, 0, '', // Empty string to add a newline when we .join('\n') below.
    ...moduleScopeLines);
  }

  if (imports !== null && imports !== void 0 && imports.length) {
    (0, _splice.default)(content).call(content, content.findLastIndex(l => (0, _includes.default)(l).call(l, '@redwoodjs')) + 1, 0, '', // Empty string to add a newline when we .join('\n') below.
    ...imports);
  }
...
function insertComponent(content, {
  ...
  (0, _splice.default)(content).call(content, open, close - open, // "Delete" the wrapped component contents. We put it back below.
  insertBefore && componentDepth + insertBefore, componentDepth + buildOpeningTag(component, props), // Increase indent of each now-nested tag by one tab (two spaces)
